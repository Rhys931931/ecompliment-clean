...
export default function Login() {
  ...
  const handleInit = async (user: any, name: string) => {
      ...
      if (!snap.exists()) {
          ...
          batch.set(doc(db, 'users', user.uid), { display_name: name, photo_url: user.photoURL, master_pin: pin, blind_key: blind, balance: 0 });
          batch.set(doc(db, 'wallets', user.uid), { balance: 0, currency: 'coins' });
          batch.set(doc(db, 'public_profiles', blind), { display_name: name, bio: 'New user' });
          await batch.commit();
      }
  };
  ...
  return (
    <div className="app-container" style={{justifyContent:'center', alignItems:'center', background:'#f0fdfa'}}>
      ...
      <div className="glass-card fade-in" style={{width:'90%', maxWidth:'400px', marginTop:'60px', background:'white'}}>
          ...
          <button onClick={handleGoogle} className="claim-btn" style={{width:'100%', justifyContent:'center', background:'white', border:'1px solid #e2e8f0'}}>
              Continue with Google
          </button>

          <p style={{textAlign:'center', marginTop:'30px', color:'#666', fontSize:'0.9rem'}}>
              {isSignUp ? "Already have an account?" : "New to eCompliment?"} 
              <span onClick={() => setIsSignUp(!isSignUp)} style={{color:'#4da6a9', fontWeight:'bold', cursor:'pointer', marginLeft:'5px'}}>
